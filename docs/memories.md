...
When `use_memories` is true, new sessions inject developer instructions built from `memory_summary.md` if the file exists and is non-empty. The model can inspect the memory workspace with normal filesystem access or the dedicated tools when enabled. If the model uses memory, it appends a hidden `<oai-mem-citation>` envelope containing a `citation_entries`, `rollout_ids`, or legacy `thread_ids` block. LHA suppresses only these structurally recognized envelopes from streaming and final UI text, parses them, and records usage for cited rollout IDs. Ordinary text that mentions the same outer tag is preserved. Structurally similar tag text is also preserved inside a top-level JSON string and inside strings in JSON objects or arrays; object and array JSON may be wrapped in prose or Markdown fences. When such text is encountered, LHA buffers the remaining JSON until the containing value validates, so that portion of streaming output may be delayed. A real citation envelope appended after valid JSON is suppressed normally, while malformed or unclosed JSON fails closed so structured citation metadata remains hidden and can still be parsed for usage recording. Once an envelope has started with a recognized inner block, an unclosed tail is still hidden and ignored so malformed metadata does not leak into visible output.
