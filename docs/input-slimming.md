...
- auto compact 同时维护 request send pressure 和 raw compaction pressure：普通模型
  ...
  compact prompt 过大时改用 slimmed compact prompt 并按需 retrieval；
...
输入。开启实验性 `retrieval_aware_compact` 后，auto compact 在普通 slimmed request
确实放不下、且 raw compact prompt 也太大时可以选择 retrieval-aware compact 作为例外。
...
- `raw_compact_pressure`：如果现在 compact，compact 模型会收到的原始 compact prompt
  token estimate；
- `retrieval_compact_pressure`：如果选择 retrieval-aware compact，compact 模型会收到的
  slimmed compact prompt token estimate。

因此 input slimming 可以降低普通请求大小；开启 `retrieval_aware_compact` 后，raw
history 超过 effective context-window 阈值不再单独触发 auto compact，只要下一次普通
slimmed request 仍能发送。remote compact 与 local compact 一样，在 raw compact prompt
已经超出 context window 时会 trim 最老 history item 后重试；如果策略明确需要
`lha_input_retrieve`，则该次 auto compact 会绕过 remote `/responses/compact`，使用本地
tool-capable compact loop。
...
- Preflight accounting：普通请求的 context-window 检查使用压缩后 request 的 token
  ...
  `retrieval_aware_compact` 会在 raw pressure-only 场景下 defer compact。
...
新增 `retrieval_aware_compact` feature key 后必须运行 `just write-config-schema`。
