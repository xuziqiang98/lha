...
# retrieval-aware auto compact fallback
retrieval_aware_compact = true

# ranked-marker auto compact fallback
ranked_marker_compact = true
```

不要同时启用 `input_slimming` 和 `input_slimming_live_zone`。`/experimental` 会在开启
其中一个 input slimming 策略时自动关闭另一个策略；如果手写 config 同时开启，runtime
会发出 warning，并禁用本次 input slimming，避免消融实验数据混淆。

`retrieval_aware_compact` 和 `ranked_marker_compact` 也是互斥的 marker-aware auto
compact 策略。`/experimental` 会在开启其中一个 compact 策略时自动关闭另一个策略；如果
手写 config 同时开启，runtime 会发出 warning，并在本次回退 raw compact。
...
- auto compact 同时维护 request send pressure 和 raw compaction pressure：普通模型
  请求可以用 slimmed request 降低 send pressure；默认 compact 仍使用原始
  `history.for_compaction_prompt()`，但实验性 `retrieval_aware_compact` 可以在 raw
  compact prompt 过大时改用 slimmed compact prompt 并按需 retrieval；实验性
  `ranked_marker_compact` 可以改用 slimmed compact prompt，但不向 compact 模型暴露
  retrieval tool，而是在 compact 后 deterministic 地追加 top-ranked durable markers；
...
live-zone slimming。实验性 retrieval-aware compact 会使用 compact 专用 wire-api label
来记录 Input Slimming metrics，不改变普通 request 的 live-zone 边界。
...
两者应长期并存。`/compact` 是会话状态操作；input slimming 是请求整形优化。默认
manual `/compact` 和默认 auto compact 使用原始 persistent history，不应用 input
slimming，不注入 `lha_input_retrieve`，也不把 `<<lha-input:...>>` replacement 作为摘要
输入。开启实验性 marker-aware compact 策略后，auto compact 在普通 slimmed request
确实放不下、且 raw compact prompt 也太大时可以选择 slimmed compact 作为例外。

auto compact 的触发和策略选择会区分三个 pressure：

- `send_pressure`：普通模型请求真正发送前的 slimmed `TurnRequest` token estimate；
- `raw_compact_pressure`：如果现在 compact，compact 模型会收到的原始 compact prompt
  token estimate；
- `marker_compact_pressure`：如果选择 retrieval-aware compact 或 ranked-marker compact，
  compact 模型会收到的 slimmed compact prompt token estimate。

因此 input slimming 可以降低普通请求大小；开启 `retrieval_aware_compact` 或
`ranked_marker_compact` 后，raw history 超过 effective context-window 阈值不再单独触发
auto compact，只要下一次普通 slimmed request 仍能发送。remote compact 与 local compact
一样，在 raw compact prompt 已经超出 context window 时会 trim 最老 history item 后重试；
marker-aware auto compact 会绕过 remote `/responses/compact`，使用本地 compact loop。

### Experimental `retrieval_aware_compact`

`retrieval_aware_compact` 默认关闭，推荐与 `input_slimming_live_zone` 一起用于调试
“大工具输出后 raw history 看起来超压，但 slimmed follow-up 实际可发送”的场景。启用后：

- turn-start 和 post-response auto compact 会优先测量下一次普通 slimmed request 的
  send pressure；
- 只有 send pressure 也超出 effective compact limit 时，才从 raw compact 和
  retrieval-aware compact 中选择策略；
...
### Experimental `ranked_marker_compact`

`ranked_marker_compact` 默认关闭，不能和 `retrieval_aware_compact` 同时开启。它适合
“希望 compact prompt 受益于 input slimming，但不希望 compact 模型额外调用 retrieval
工具”的实验场景。启用后：

- auto compact 仍先按 send pressure 和 raw compact pressure 判断是否需要 compact；
- ranked-marker compact 的 prompt 会先经过 Input Slimming，但不会暴露
  `lha_input_retrieve`，`tools` 为空，`parallel_tool_calls` 为 false；
- compact 模型只总结可见 compressed snippets 和任务上下文，不应臆造 marker 背后的原文；
- compact 完成后，runtime 会对 compact source 中出现、且当前 store 或 rollout sidecar
  可恢复的 durable markers 评分排序；
- summary 末尾只追加 `Ranked retrievable markers:` 小节中的 top-ranked markers，不追加
  原始工具输出文本；
- retention 同时受 `RANKED_MARKER_COMPACT_MAX_MARKERS = 12` 和
  `RANKED_MARKER_COMPACT_MARKER_SECTION_MAX_TOKENS = 900` 限制；
- scoring factors 包含 retrieval count、最新出现位置、失败/错误信号、压缩损失、工具名
  优先级、出现次数、字节熵、文本质量和 blob-like noise penalty；
- 手动 `/compact` 不受影响，仍走 raw compact。
...
`ranked_marker_compact`，因此 `config.schema.json` 已同步更新。
...
- Preflight accounting：普通请求的 context-window 检查使用压缩后 request 的 token
  estimate；默认 auto compact 另行检查 raw compaction pressure；实验性
  `retrieval_aware_compact` 和 `ranked_marker_compact` 会在 raw pressure-only 场景下
  defer compact。
- Ranked-marker compact：compact prompt 使用 input-slimmed replacements、不暴露
  `lha_input_retrieve`，compact 后只追加 top-ranked durable markers，并覆盖 retrieval
  count boost、top-k retention、raw original text 不进入 summary 等场景。
...
新增或调整 `retrieval_aware_compact` / `ranked_marker_compact` 等 feature key 后必须运行
`just write-config-schema`。
