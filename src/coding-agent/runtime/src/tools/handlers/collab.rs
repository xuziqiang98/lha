...
use crate::subagents::SpawnAgentOptions;
use crate::subagents::exceeds_thread_spawn_depth_limit;
use crate::subagents::next_thread_spawn_depth;
use crate::subagents::role::DEFAULT_ROLE_NAME;
use crate::subagents::role::apply_role_to_config;
...
use adam_protocol::protocol::CollabAgentInteractionBeginEvent;
use adam_protocol::protocol::CollabAgentInteractionEndEvent;
use adam_protocol::protocol::CollabAgentRef;
use adam_protocol::protocol::CollabAgentSpawnBeginEvent;
use adam_protocol::protocol::CollabAgentSpawnEndEvent;
use adam_protocol::protocol::CollabAgentStatusEntry;
use adam_protocol::protocol::CollabCloseBeginEvent;
use adam_protocol::protocol::CollabCloseEndEvent;
use adam_protocol::protocol::CollabResumeBeginEvent;
use adam_protocol::protocol::CollabResumeEndEvent;
use adam_protocol::protocol::CollabWaitingBeginEvent;
use adam_protocol::protocol::CollabWaitingEndEvent;
...
mod spawn {
    ...
    pub async fn handle(
        ...
        let session_source = turn.runtime.get_session_source();
        let child_depth = next_thread_spawn_depth(&session_source);
        if exceeds_thread_spawn_depth_limit(child_depth, turn.runtime.config().agent_max_depth) {
            return Err(FunctionCallError::RespondToModel(
...
fn build_agent_shared_config(turn: &TurnContext) -> Result<Config, FunctionCallError> {
    let base_config = turn.runtime.config();
    let mut config = (*base_config).clone();
    config.model = Some(turn.runtime.get_model());
    config.model_provider = turn.runtime.endpoint();
    config.model_reasoning_effort = turn.runtime.get_reasoning_effort();
    config.model_reasoning_summary = turn.runtime.get_reasoning_summary();
...
mod tests {
    ...
    async fn wait_for_agent_status(
        agent_control: &crate::subagents::AgentControl,
        thread_id: ThreadId,
    ...
    async fn spawn_agent_rejects_when_depth_limit_exceeded() {
        ...
        let max_depth = turn.runtime.config().agent_max_depth;
        ...
        turn.runtime = turn.runtime.derive_runtime(
            turn.runtime.config(),
            turn.runtime.get_model_info(),
            turn.runtime.dynamic_context_window(),
            turn.runtime.get_otel_manager(),
            turn.runtime.get_reasoning_effort(),
            turn.runtime.get_reasoning_summary(),
            session_source,
        );
    ...
    async fn send_input_interrupts_before_prompt() {
        ...
        let manager = thread_manager();
        session.services.agent_control = manager.agent_control();
        let config = turn.runtime.config().as_ref().clone();
    ...
    async fn build_agent_spawn_config_uses_turn_context_values() {
        ...
        let mut expected = (*turn.runtime.config()).clone();
        expected.base_instructions = Some(base_instructions.text);
        expected.model = Some(turn.runtime.get_model());
        expected.model_provider = turn.runtime.endpoint();
        expected.model_reasoning_effort = turn.runtime.get_reasoning_effort();
        expected.model_reasoning_summary = turn.runtime.get_reasoning_summary();
