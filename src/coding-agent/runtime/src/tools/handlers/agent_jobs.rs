...
impl JobProgressEmitter {
    ...
    async fn maybe_emit(
        ...
        turn: &TurnContext,
        job_id: &str,
        progress: &adam_state::AgentJobProgress,
        force: bool,
    ) -> anyhow::Result<()> {
...
mod spawn_agents_on_csv {
    ...
    pub async fn handle(
        ...
        let (headers, rows) = parse_csv(&csv_content).map_err(|err| {
        ...
        for (idx, row) in rows.into_iter().enumerate() {
            ...
            items.push(adam_state::AgentJobItemCreateParams {
                item_id,
        ...
        let max_runtime_seconds = normalize_max_runtime_seconds(
            args.max_runtime_seconds
                .or(turn.runtime.config().agent_job_max_runtime_seconds),
        )?;
        let _job = db
            .create_agent_job(
                &adam_state::AgentJobCreateParams {
                    id: job_id.clone(),
                    name: job_name,
        ...
        let max_threads = turn.runtime.config().agent_max_threads;
        ...
        let failed_item_errors = if progress.failed_items > 0 {
            let items = db
                .list_agent_job_items(
                    job_id.as_str(),
                    Some(adam_state::AgentJobItemStatus::Failed),
                    Some(5),
                )
                .await
                .unwrap_or_default();
...
fn required_state_db(
    session: &Arc<Session>,
) -> Result<Arc<adam_state::StateRuntime>, FunctionCallError> {
...
async fn build_runner_options(
    ...
    let max_concurrency = normalize_concurrency(
        requested_concurrency,
        turn.runtime.config().agent_max_threads,
    );
...
async fn run_agent_job_loop(
    session: Arc<Session>,
    turn: Arc<TurnContext>,
    db: Arc<adam_state::StateRuntime>,
    job_id: String,
    ...
    loop {
        ...
        if !cancel_requested && active_items.len() < options.max_concurrency {
            ...
            let pending_items = db
                .list_agent_job_items(
                    job_id.as_str(),
                    Some(adam_state::AgentJobItemStatus::Pending),
                    Some(slots),
                )
                .await?;
    ...
    if progress.failed_items > 0 {
        ...
        session
            .notify_background_event(
                &turn,
                format!("agent job completed with {failed_items} failed items"),
            )
            .await;
    }
...
async fn recover_running_items(
    ...
    let running_items = db
        .list_agent_job_items(job_id, Some(adam_state::AgentJobItemStatus::Running), None)
        .await?;
...
fn active_item_status_from_receiver(status_rx: &mut Receiver<AgentStatus>) -> Option<AgentStatus> {
    status_rx.has_changed().ok()?;
...
async fn finalize_finished_item(
    ...
    if matches!(item.status, adam_state::AgentJobItemStatus::Running) {
...
fn job_runtime_timeout(job: &adam_state::AgentJob) -> Duration {
    job.max_runtime_seconds
        .map(Duration::from_secs)
        .unwrap_or(DEFAULT_AGENT_JOB_ITEM_TIMEOUT)
}

fn started_at_from_item(item: &adam_state::AgentJobItem) -> Instant {
...
fn is_item_stale(item: &adam_state::AgentJobItem, runtime_timeout: Duration) -> bool {
    let now = chrono::Utc::now();
...
mod tests {
    ...
    #[test]
    fn active_item_status_from_receiver_returns_cached_status_when_open() {
        let (tx, mut rx) = watch::channel(AgentStatus::Running);
        assert_eq!(
            active_item_status_from_receiver(&mut rx),
    ...
    #[test]
    fn active_item_status_from_receiver_returns_none_when_sender_closed() {
        let (tx, mut rx) = watch::channel(AgentStatus::Running);
        drop(tx);
        assert_eq!(active_item_status_from_receiver(&mut rx), None);
    }

    #[test]
    fn active_item_status_from_receiver_marks_pending_update_as_seen() {
        let (tx, mut rx) = watch::channel(AgentStatus::PendingInit);
        tx.send(AgentStatus::Running).expect("send running status");

        assert_eq!(
            active_item_status_from_receiver(&mut rx),
            Some(AgentStatus::Running)
        );
        assert!(!rx.has_changed().expect("status receiver still open"));
    }
}
