...
use codex_agent::protocol::CollabAgentRef;
use codex_agent::protocol::CollabAgentSpawnEndEvent;
use codex_agent::protocol::CollabAgentStatusEntry;
...
pub(crate) fn waiting_begin(ev: CollabWaitingBeginEvent) -> PlainHistoryCell {
    ...
    let details = if receiver_agents.len() > 1 {
        receiver_agents
            .iter()
            .map(agent_label_from_ref)
            .map(agent_label_line)
            .collect()
    } else {
        Vec::new()
    };
...
fn spawn_request_spans(spawn_request: Option<&SpawnRequestSummary>) -> Vec<Span<'static>> {
    ...
    vec![" ".dim(), Span::from(details).magenta()]
}
...
fn wait_complete_lines(
    ...
    let entries = if agent_statuses.is_empty() {
        ...
        entries.sort_by(|left, right| {
            left.agent
                .thread_id
                .to_string()
                .cmp(&right.agent.thread_id.to_string())
        });
        entries
    } else {
        ...
        let seen = entries
            .iter()
            .map(|entry| entry.agent.thread_id)
            .collect::<HashSet<_>>();
    ...
    entries
        .into_iter()
        .map(|entry| {
            let mut spans = agent_label_spans(agent_label_from_ref(&entry.agent));
            spans.push(": ".dim());
...
mod tests {
    ...
    #[test]
    fn title_styles_nickname_role_and_spawn_request() {
        let sender_thread_id = ThreadId::from_string("00000000-0000-0000-0000-000000000001")
            .expect("valid sender thread id");
        let robie_id = ThreadId::from_string("00000000-0000-0000-0000-000000000002")
            .expect("valid robie thread id");
