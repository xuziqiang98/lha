...
pub fn built_in_runtime_endpoints() -> HashMap<String, RuntimeEndpoint> {
    ...
    [
        ...
        (
            ...
            create_oss_endpoint(DEFAULT_OLLAMA_PORT, ConversationDialect::Responses),
        ),
        (
            OLLAMA_CHAT_PROVIDER_ID,
            create_oss_endpoint(DEFAULT_OLLAMA_PORT, ConversationDialect::Chat),
        ),
        (
            LMSTUDIO_OSS_PROVIDER_ID,
            create_oss_endpoint(DEFAULT_LMSTUDIO_PORT, ConversationDialect::Responses),
        ),
    ]
    .into_iter()
...
fn create_oss_endpoint(
    ...
    create_oss_endpoint_with_base_url(&codex_oss_base_url, dialect)
}

fn create_oss_endpoint_with_base_url(
    base_url: &str,
    dialect: ConversationDialect,
) -> RuntimeEndpoint {
    RuntimeEndpoint::new_custom("gpt-oss".into(), Some(base_url.into()), dialect)
}
