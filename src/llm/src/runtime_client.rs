...
use crate::api::ChatRequestBuilder as ApiChatRequestBuilder;
use crate::api::CompactClient as ApiCompactClient;
use crate::api::CompactionInput as ApiCompactionInput;
use crate::api::DeveloperRoleHandling;
use crate::api::MessagesClient as ApiMessagesClient;
use crate::api::MessagesRequestBuilder as ApiMessagesRequestBuilder;
use crate::api::Prompt as ApiPrompt;
use crate::api::RequestTelemetry;
use crate::api::ReqwestTransport;
use crate::api::ResponseAppendWsRequest;
use crate::api::ResponseCreateWsRequest;
...
use crate::api::ResponsesWebsocketClient as ApiWebSocketResponsesClient;
use crate::api::ResponsesWebsocketConnection as ApiWebSocketConnection;
use crate::api::SseTelemetry;
use crate::api::TransportError;
use crate::api::WebsocketTelemetry;
...
impl LlmClientSession {
    ...
    async fn websocket_connection(
        &mut self,
        api_provider: crate::api::Provider,
    ...
    async fn stream_messages_api(&self, prompt: &Prompt) -> Result<ResponseStream> {
        ...
        let api_auth = auth_provider_from_endpoint(&self.state.endpoint)?;
        {
            ...
            match client.stream_request(request).await {
                Ok(stream) => Ok(map_response_stream(stream, self.state.telemetry.clone())),
                Err(ApiError::Transport(TransportError::Http { status, .. }))
                    if status == StatusCode::UNAUTHORIZED =>
                {
                    Err(unauthorized_error(status))
    ...
    async fn stream_responses_api(&self, prompt: &Prompt) -> Result<ResponseStream> {
        if let Some(path) = &self.state.runtime_config.sse_fixture_path {
            ...
            let stream =
                crate::api::stream_from_fixture(path, self.state.endpoint.stream_idle_timeout())?;
            return Ok(map_response_stream(stream, self.state.telemetry.clone()));
        }
    ...
    fn build_streaming_telemetry(&self) -> (Arc<dyn RequestTelemetry>, Arc<dyn SseTelemetry>) {
        let telemetry = Arc::new(ApiTelemetry::new(self.state.telemetry.clone()));
...
) -> std::result::Result<crate::api::ChatRequest, Error> {
...
fn map_response_stream<S>(api_stream: S, telemetry: Arc<dyn RuntimeTelemetry>) -> ResponseStream
where
            ...
            match event {
                Ok(ResponseEvent::Completed {
                    ...
                    if let Some(usage) = &token_usage {
                        telemetry.record_response_completed(
                            usage.input_tokens,
                            usage.output_tokens,
                ...
                Err(err) => {
                    ...
                    if !logged_error {
                        telemetry.record_response_completed_failed(&mapped.to_string());
                        logged_error = true;
                    }
...
impl ApiTelemetry {
    fn new(telemetry: Arc<dyn RuntimeTelemetry>) -> Self {
        Self { telemetry }
    }
}
...
impl SseTelemetry for ApiTelemetry {
    fn on_sse_poll(
        ...
        record_sse_poll(self.telemetry.as_ref(), result, duration);
    }
}
...
impl WebsocketTelemetry for ApiTelemetry {
    ...
    fn on_ws_event(
        ...
        record_websocket_event(self.telemetry.as_ref(), result, duration);
    }
}

fn record_sse_poll(
    telemetry: &dyn RuntimeTelemetry,
    result: &std::result::Result<
        Option<std::result::Result<Event, EventStreamError<TransportError>>>,
        tokio::time::error::Elapsed,
    >,
    ...
    let (kind, success, error) = match result {
        Ok(Some(Ok(sse))) if sse.data.trim() == "[DONE]" => (Some(sse.event.as_str()), true, None),
        Ok(Some(Ok(sse))) => match serde_json::from_str::<Value>(&sse.data) {
            Ok(value) if sse.event == "response.failed" => {
                (Some(sse.event.as_str()), false, Some(value.to_string()))
            }
            Ok(_) => (Some(sse.event.as_str()), true, None),
            Err(error) => (Some(sse.event.as_str()), false, Some(error.to_string())),
        },
        Ok(Some(Err(error))) => (None, false, Some(error.to_string())),
        Ok(None) => return,
        Err(_) => (
            None,
            false,
    ...
    telemetry.record_sse_event(kind, success, error.as_deref(), duration);
}

fn record_websocket_event(
    telemetry: &dyn RuntimeTelemetry,
    result: &std::result::Result<Option<std::result::Result<Message, WebsocketError>>, ApiError>,
    duration: Duration,
) {
    let mut kind = None;
    ...
    telemetry.record_websocket_event(kind.as_deref(), success, error_message.as_deref(), duration);
}
