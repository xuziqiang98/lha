...
pub struct PlanRunUpdate {
    ...
    pub status: Option<crate::ThreadPlanRunStatus>,
    pub token_budget: Option<Option<i64>>,
    pub expected_plan_run_id: Option<String>,
}
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ThreadPlanRunSeed {
    pub plan_text: String,
    pub status: crate::ThreadPlanRunStatus,
    pub token_budget: Option<i64>,
    pub tokens_used: i64,
    pub time_used_seconds: i64,
    pub created_at: DateTime<Utc>,
...
pub enum PlanRunAccountingOutcome {
    Unchanged(Option<crate::ThreadPlanRun>),
    Updated(crate::ThreadPlanRun),
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum PlanRunAccountingMode {
    ActiveStatusOnly,
    ActiveOnly,
    ActiveOrComplete,
    ActiveOrStopped,
...
RETURNING
    ...
    pub async fn get_thread_plan_run(
        &self,
        thread_id: ThreadId,
    ) -> anyhow::Result<Option<crate::ThreadPlanRun>> {
        let row = sqlx::query(
            r#"
SELECT
    thread_id,
...
WHERE thread_id = ?
        ...
        row.map(|row| ThreadPlanRunRow::try_from_row(&row).and_then(crate::ThreadPlanRun::try_from))
            .transpose()
    }

    pub async fn replace_thread_plan_run(
        &self,
        thread_id: ThreadId,
        plan_text: &str,
        status: crate::ThreadPlanRunStatus,
        token_budget: Option<i64>,
    ) -> anyhow::Result<crate::ThreadPlanRun> {
        let plan_run_id = Uuid::new_v4().to_string();
        let now_ms = datetime_to_epoch_millis(Utc::now());
        let status = status_after_plan_run_budget_limit(status, 0, token_budget);
        let row = sqlx::query(
            r#"
INSERT INTO thread_plan_runs (
    thread_id,
    plan_run_id,
    plan_text,
    status,
...
RETURNING
    ...
    updated_at_ms
        ...
        ThreadPlanRunRow::try_from_row(&row).and_then(crate::ThreadPlanRun::try_from)
    }

    pub async fn replace_thread_plan_run_if_plan_run_id(
        &self,
        thread_id: ThreadId,
...
RETURNING
    ...
    pub async fn insert_thread_plan_run(
        &self,
        thread_id: ThreadId,
        plan_text: &str,
        status: crate::ThreadPlanRunStatus,
...
RETURNING
    ...
    pub async fn insert_thread_plan_run_or_replace_completed(
        &self,
        thread_id: ThreadId,
        plan_text: &str,
        status: crate::ThreadPlanRunStatus,
...
RETURNING
    ...
    updated_at_ms
        ...
        .bind(now_ms)
        .bind(now_ms)
        .bind(crate::ThreadPlanRunStatus::Complete.as_str())
        .fetch_optional(self.pool.as_ref())
        .await?;
    ...
    pub async fn seed_thread_plan_run(
        &self,
        thread_id: ThreadId,
        seed: ThreadPlanRunSeed,
    ) -> anyhow::Result<crate::ThreadPlanRun> {
        let plan_run_id = Uuid::new_v4().to_string();
        let tokens_used = seed.tokens_used.max(0);
        let time_used_seconds = seed.time_used_seconds.max(0);
        ...
        let updated_at_ms = datetime_to_epoch_millis(seed.updated_at);
        let status =
            status_after_plan_run_budget_limit(seed.status, tokens_used, seed.token_budget);
        let row = sqlx::query(
            r#"
INSERT INTO thread_plan_runs (
    thread_id,
...
RETURNING
    ...
    pub async fn update_thread_plan_run(
        &self,
        thread_id: ThreadId,
        ...
        let PlanRunUpdate {
            ...
            status,
            token_budget,
            expected_plan_run_id,
        } = update;
        let plan_text = plan_text.as_deref();
        let expected_plan_run_id = expected_plan_run_id.as_deref();
        let now_ms = datetime_to_epoch_millis(Utc::now());
        let result = match (status, token_budget) {
            (Some(status), Some(token_budget)) => {
                sqlx::query(
...
SET
    ...
    updated_at_ms = ?
WHERE thread_id = ?
  AND (? IS NULL OR plan_run_id = ?)
            "#,
                )
                .bind(plan_text)
                .bind(crate::ThreadPlanRunStatus::BudgetLimited.as_str())
                .bind(status.as_str())
                .bind(crate::ThreadPlanRunStatus::Paused.as_str())
                .bind(crate::ThreadPlanRunStatus::Blocked.as_str())
                .bind(status.as_str())
                .bind(token_budget)
                .bind(token_budget)
                .bind(crate::ThreadPlanRunStatus::BudgetLimited.as_str())
...
WHERE thread_id = ?
  AND (? IS NULL OR plan_run_id = ?)
            "#,
                    ...
                    let plan_run = self.get_thread_plan_run(thread_id).await?;
                    return Ok(match (plan_run, expected_plan_run_id) {
                        (Some(plan_run), Some(expected_plan_run_id))
                            if plan_run.plan_run_id != expected_plan_run_id =>
                        {
                            None
                        }
                        (plan_run, None) | (plan_run, Some(_)) => plan_run,
                    });
                }
            }
        };
        ...
        self.get_thread_plan_run(thread_id).await
    }

    pub async fn pause_active_thread_plan_run(
        &self,
        thread_id: ThreadId,
    ) -> anyhow::Result<Option<crate::ThreadPlanRun>> {
        self.pause_active_thread_plan_run_if_plan_run_id(thread_id, None)
            .await
    }

    pub async fn pause_active_thread_plan_run_if_plan_run_id(
        &self,
        thread_id: ThreadId,
        expected_plan_run_id: Option<&str>,
    ) -> anyhow::Result<Option<crate::ThreadPlanRun>> {
        self.update_active_thread_plan_run_status(
            thread_id,
            crate::ThreadPlanRunStatus::Paused,
            expected_plan_run_id,
        )
        .await
    }

    pub async fn usage_limit_active_thread_plan_run(
        &self,
        thread_id: ThreadId,
    ) -> anyhow::Result<Option<crate::ThreadPlanRun>> {
        self.update_active_thread_plan_run_status(
            thread_id,
            crate::ThreadPlanRunStatus::UsageLimited,
            None,
        )
        .await
    }

    async fn update_active_thread_plan_run_status(
        &self,
        thread_id: ThreadId,
        status: crate::ThreadPlanRunStatus,
        expected_plan_run_id: Option<&str>,
...
WHERE thread_id = ?
  ...
  AND (? IS NULL OR plan_run_id = ?)
    ...
    pub async fn account_thread_plan_run_usage(
        &self,
        thread_id: ThreadId,
        time_delta_seconds: i64,
        token_delta: i64,
        mode: PlanRunAccountingMode,
        expected_plan_run_id: Option<&str>,
    ) -> anyhow::Result<PlanRunAccountingOutcome> {
        let time_delta_seconds = time_delta_seconds.max(0);
        let token_delta = token_delta.max(0);
        if time_delta_seconds == 0 && token_delta == 0 {
            return Ok(PlanRunAccountingOutcome::Unchanged(
                self.get_thread_plan_run(thread_id).await?,
            ));
        }
        ...
        let now_ms = datetime_to_epoch_millis(Utc::now());
        let status_filter = match mode {
            PlanRunAccountingMode::ActiveStatusOnly => "status = 'active'",
            PlanRunAccountingMode::ActiveOnly => "status IN ('active', 'budget_limited')",
            PlanRunAccountingMode::ActiveOrComplete => {
                "status IN ('active', 'budget_limited', 'complete')"
            }
            PlanRunAccountingMode::ActiveOrStopped => {
                "status IN ('active', 'paused', 'blocked', 'usage_limited', 'budget_limited')"
            }
        };
        let budget_limit_status_filter = match mode {
            PlanRunAccountingMode::ActiveStatusOnly
            | PlanRunAccountingMode::ActiveOnly
            | PlanRunAccountingMode::ActiveOrComplete => "status = 'active'",
            PlanRunAccountingMode::ActiveOrStopped => {
                "status IN ('active', 'paused', 'blocked', 'usage_limited', 'budget_limited')"
            }
        };
        let plan_run_id_filter = if expected_plan_run_id.is_some() {
            "plan_run_id = ?"
        } else {
            "1 = 1"
        };
...
RETURNING
    ...
    updated_at_ms
        ...
        let mut query = sqlx::query(&query)
            ...
            .bind(now_ms)
            .bind(thread_id.to_string());
        if let Some(expected_plan_run_id) = expected_plan_run_id {
            query = query.bind(expected_plan_run_id);
        }
        ...
        let updated =
            ThreadPlanRunRow::try_from_row(&row).and_then(crate::ThreadPlanRun::try_from)?;
        Ok(PlanRunAccountingOutcome::Updated(updated))
    }

    pub async fn delete_thread_plan_run(&self, thread_id: ThreadId) -> anyhow::Result<bool> {
        let result = sqlx::query("DELETE FROM thread_plan_runs WHERE thread_id = ?")
            .bind(thread_id.to_string())
            .execute(self.pool.as_ref())
            .await?;
        Ok(result.rows_affected() > 0)
...
fn status_after_plan_run_budget_limit(
    status: crate::ThreadPlanRunStatus,
    tokens_used: i64,
    token_budget: Option<i64>,
) -> crate::ThreadPlanRunStatus {
    if status == crate::ThreadPlanRunStatus::Active
        && token_budget.is_some_and(|budget| tokens_used >= budget)
    {
        crate::ThreadPlanRunStatus::BudgetLimited
    } else {
        status
    }
}
...
mod tests {
    ...
    #[tokio::test]
    async fn replace_update_insert_and_delete_thread_plan_run() {
        let runtime = test_runtime().await;
        let thread_id = test_thread_id();

        let plan_run = runtime
            .replace_thread_plan_run(
                thread_id,
                "- implement plan",
                crate::ThreadPlanRunStatus::Active,
                None,
            )
            .await
        ...
        let updated = runtime
            .update_thread_plan_run(
                thread_id,
                PlanRunUpdate {
                    plan_text: Some("- updated plan".to_string()),
                    status: Some(crate::ThreadPlanRunStatus::Paused),
                    token_budget: Some(Some(20)),
                    expected_plan_run_id: Some(plan_run.plan_run_id.clone()),
                },
            )
            .await
            .expect("plan run update should succeed")
            .expect("plan run should exist");
        assert_eq!(
            crate::ThreadPlanRun {
                plan_text: "- updated plan".to_string(),
                status: crate::ThreadPlanRunStatus::Paused,
        ...
        assert!(
            runtime
                .delete_thread_plan_run(thread_id)
                .await
                .expect("plan run delete should succeed")
        );
        assert_eq!(
            None,
            runtime
    ...
    #[tokio::test]
    async fn update_thread_plan_run_rejects_stale_plan_run_id() {
        let runtime = test_runtime().await;
        let thread_id = test_thread_id();
        ...
        let updated = runtime
            .update_thread_plan_run(
                ...
                PlanRunUpdate {
                    plan_text: None,
                    status: Some(crate::ThreadPlanRunStatus::Complete),
                    token_budget: None,
                    expected_plan_run_id: Some("stale-plan-run-id".to_string()),
                },
            )
            .await
            .expect("plan run update should succeed");
    ...
    #[tokio::test]
    async fn account_thread_plan_run_usage_updates_usage_and_budget_status() {
        let runtime = test_runtime().await;
        let thread_id = test_thread_id();
        ...
        let first_update = runtime
            .account_thread_plan_run_usage(
                ...
                7,
                PlanRunAccountingMode::ActiveStatusOnly,
                Some(plan_run.plan_run_id.as_str()),
            )
            .await
            .expect("plan run accounting should succeed");
        let PlanRunAccountingOutcome::Updated(first_update) = first_update else {
            panic!("expected plan run accounting update");
        };
        assert_eq!(
            crate::ThreadPlanRun {
                tokens_used: 7,
                time_used_seconds: 3,
                updated_at: first_update.updated_at,
                ..plan_run.clone()
            },
            first_update
        );
        ...
        let second_update = runtime
            ...
            .await
            .expect("plan run accounting should succeed");
        let PlanRunAccountingOutcome::Updated(second_update) = second_update else {
            panic!("expected plan run accounting update");
        };
        assert_eq!(
            crate::ThreadPlanRun {
                status: crate::ThreadPlanRunStatus::BudgetLimited,
                tokens_used: 15,
                time_used_seconds: 4,
                updated_at: second_update.updated_at,
                ..plan_run
            },
            second_update
        );
    ...
    #[tokio::test]
    async fn seed_thread_plan_run_rewrites_thread_and_plan_run_ids() {
        let runtime = test_runtime().await;
        let thread_id = test_thread_id();
        ...
        let original = runtime
            .replace_thread_plan_run(
                thread_id,
                "old plan",
                crate::ThreadPlanRunStatus::Complete,
                None,
            )
            .await
            .expect("plan run replacement should succeed");
        ...
        let seeded = runtime
            .seed_thread_plan_run(
                thread_id,
                ThreadPlanRunSeed {
                    plan_text: "seeded plan".to_string(),
                    status: crate::ThreadPlanRunStatus::Active,
                    token_budget: Some(5),
                    tokens_used: 10,
            ...
            .await
            .expect("plan run seed should succeed");

        assert_ne!(original.plan_run_id, seeded.plan_run_id);
        let expected = crate::ThreadPlanRun {
            thread_id,
            plan_run_id: seeded.plan_run_id.clone(),
            plan_text: "seeded plan".to_string(),
            status: crate::ThreadPlanRunStatus::BudgetLimited,
            token_budget: Some(5),
            tokens_used: 10,
    ...
    #[tokio::test]
    async fn insert_thread_plan_run_or_replace_completed_blocks_unfinished_plan_run() {
        let runtime = test_runtime().await;
        let thread_id = test_thread_id();
    ...
    #[tokio::test]
    async fn insert_thread_plan_run_or_replace_completed_replaces_completed_plan_run() {
        let runtime = test_runtime().await;
        let thread_id = test_thread_id();
        ...
        runtime
            .update_thread_plan_run(
                ...
                PlanRunUpdate {
                    ...
                    status: Some(crate::ThreadPlanRunStatus::Complete),
                    token_budget: None,
                    expected_plan_run_id: Some(original.plan_run_id.clone()),
                },
            )
            .await
            .expect("plan run update should succeed")
        ...
        let replacement = runtime
            ...
            .await
            .expect("conditional insert should not fail")
            .expect("completed plan run should be replaced");

        assert_ne!(original.plan_run_id, replacement.plan_run_id);
        let expected = crate::ThreadPlanRun {
            thread_id,
            plan_run_id: replacement.plan_run_id.clone(),
            plan_text: "start new plan".to_string(),
            status: crate::ThreadPlanRunStatus::Active,
            token_budget: None,
            tokens_used: 0,
            time_used_seconds: 0,
