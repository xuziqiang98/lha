...
use adam_backend_client as backend;
use adam_backend_client::CodeTaskDetailsResponseExt;
...
mod api {
    ...
    impl<'a> Apply<'a> {
        ...
        pub(crate) async fn run(
            ...
            let req = adam_git::ApplyGitRequest {
                cwd: std::env::current_dir().unwrap_or_else(|_| std::env::temp_dir()),
                diff: diff.clone(),
                revert: false,
                preflight,
            };
            let r = adam_git::apply_git_patch(&req)
