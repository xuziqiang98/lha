...
ChatGPT account login, account inspection, logout, and account rate-limit JSON-RPC endpoints are no longer supported. Configure model providers with API-key environment variables such as `OPENAI_API_KEY`, or use MCP OAuth through the `mcpServer/oauth/*` methods for MCP servers.
