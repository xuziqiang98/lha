...
`adam-agent` sits above `adam-llm` and below user-facing surfaces such as `adam-tui`, `adam-exec`, and `codex app-server`.
...
- `adam-agent` owns Codex-specific agent behavior, tool orchestration, config, prompts, and adapters on top of that kernel.
