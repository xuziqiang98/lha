...
use lha_protocol::request_user_input::RequestUserInputAnswer;
    ...
    F: FnOnce(String) -> Identity,
{
    ...
    let session_model = session_configured.model.clone();
    let identity = build_identity(session_model.clone());
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn request_user_input_rejected_in_programmer_identity() -> anyhow::Result<()> {
    assert_request_user_input_rejected("programmer", |model| Identity {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn request_user_input_rejected_in_nobody_identity() -> anyhow::Result<()> {
    assert_request_user_input_rejected("nobody", |model| Identity {
