...
pub(crate) fn map_session_init_error(err: &anyhow::Error, adam_home: &Path) -> CodexErr {
    if let Some(mapped) = err
        .chain()
        .filter_map(|cause| cause.downcast_ref::<std::io::Error>())
        .find_map(|io_err| map_rollout_io_error(io_err, adam_home))
    {
        return mapped;
    }
...
fn map_rollout_io_error(io_err: &std::io::Error, adam_home: &Path) -> Option<CodexErr> {
    ...
    let hint = match io_err.kind() {
        ...
        ErrorKind::NotFound => format!(
            "Session storage missing at {}. Create the directory or choose a different Adam home.",
            sessions_dir.display()
        ),
