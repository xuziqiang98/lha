...
Set a stable path to the skill CLI (default `LHA_HOME` is `~/.lha`):

```
export LHA_HOME="${LHA_HOME:-$HOME/.lha}"
export IMAGE_GEN="$LHA_HOME/skills/.system/imagegen/scripts/image_gen.py"
```
...
- Do not silently downgrade from CLI `gpt-image-2` or built-in `imagegen` to CLI `gpt-image-1.5`; ask first unless the user already explicitly requested `gpt-image-1.5`, `scripts/image_gen.py`, or CLI fallback.
...
When using this path, explain briefly that built-in `imagegen` plus chroma-key removal is the default transparent-image path, but this request needs true model-native transparency. `gpt-image-2` does not support `background=transparent`, so `gpt-image-1.5` is required for this confirmed fallback.
...
- Network/sandbox notes for fallback CLI mode: `references/lha-network.md`
- Built-in-first transparent image workflow: `SKILL.md` and `$LHA_HOME/skills/.system/imagegen/scripts/remove_chroma_key.py`
