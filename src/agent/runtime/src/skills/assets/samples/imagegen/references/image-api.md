...
`gpt-image-2` does not currently support the Image API `background=transparent` parameter. The skill's default transparent-image path is built-in `imagegen` with a flat chroma-key background, followed by local alpha extraction with `python "${LHA_HOME:-$HOME/.lha}/skills/.system/imagegen/scripts/remove_chroma_key.py"`.
