...
use crate::memories::runtime::SpawnedConsolidationAgent;
use crate::memories::runtime::disable_consolidation_features;
...
pub(crate) async fn run(context: Arc<MemoryStartupContext>) {
    ...
    if let Err(err) = lha_memories_write::prepare_memory_workspace(memory_root.as_path()).await {
    ...
    let workspace_diff =
        match lha_memories_write::memory_workspace_diff(memory_root.as_path()).await {
            Ok(diff) => diff,
            Err(err) => {
    ...
    if let Err(err) =
        lha_memories_write::write_workspace_diff(memory_root.as_path(), &workspace_diff).await
    {
    ...
    let prompt = lha_memories_write::build_consolidation_prompt(memory_root.as_path());
    ...
    if success {
            ...
            .await
        {
            Ok(true) => {
                if let Err(err) =
                    lha_memories_write::reset_memory_workspace_baseline(memory_root.as_path()).await
                {
...
async fn sync_workspace_inputs(
    ...
    lha_memories_write::sync_rollout_summaries_from_memories(
        root,
        selected_outputs,
        max_raw_memories,
    )
    .await?;
    lha_memories_write::rebuild_raw_memories_file_from_memories(
        root,
        selected_outputs,
        max_raw_memories,
    )
...
fn consolidation_config(
    ...
    agent_config.model_reasoning_effort =
        Some(lha_protocol::openai_models::ReasoningEffort::Medium);
    Ok(agent_config)
}
...
async fn run_consolidation_agent(
    ...
    let mut heartbeat_interval = tokio::time::interval(Duration::from_secs(
        lha_memories_write::STAGE_TWO_JOB_HEARTBEAT_SECONDS,
    ));
...
async fn mark_failed(
    ...
    match state_db
        ...
        .mark_global_phase2_job_failed(
            &claim.token,
            reason,
            lha_memories_write::STAGE_TWO_JOB_RETRY_DELAY_SECONDS,
        )
        .await
    {
        Ok(true) => {}
    ...
    fn consolidation_config_disables_external_context_and_memory_features() {
        ...
        config.mcp_servers = Constrained::allow_any(HashMap::from([(
            ...
            crate::config::types::McpServerConfig {
                transport: crate::config::types::McpServerTransportConfig::Stdio {
