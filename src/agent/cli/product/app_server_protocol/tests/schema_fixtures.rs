use crate::product::app_server_protocol::read_schema_fixture_tree;
use crate::product::app_server_protocol::write_schema_fixtures;
use anyhow::Context;
use anyhow::Result;
...
fn schema_root() -> Result<std::path::PathBuf> {
    ...
    let typescript_index = crate::test_support::cargo_bin::find_resource!(
        "product/app_server_protocol/schema/typescript/index.ts"
    )
    ...
    let json_bundle = crate::test_support::cargo_bin::find_resource!(
        "product/app_server_protocol/schema/json/lha_app_server_protocol.schemas.json"
    )
