...
use crate::test_support::app_server::write_mock_responses_models_json;
use crate::test_support::app_server::write_state_json;
use anyhow::Result;
...
async fn thread_unarchive_moves_rollout_back_into_sessions_directory() -> Result<()> {
    ...
    let archived_path = find_archived_thread_path_by_id_str(lha_home.path(), &thread.id)
        .await?
