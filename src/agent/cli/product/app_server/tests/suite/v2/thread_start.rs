...
async fn thread_start_respects_request_reasoning_effort() -> Result<()> {
    ...
    let req_id = mcp
        .send_turn_start_request(crate::product::app_server_protocol::TurnStartParams {
            thread_id: thread.id,
            effort: Some(ReasoningEffort::High),
            input: vec![crate::product::app_server_protocol::UserInput::Text {
    ...
    .await??;
    let _: crate::product::app_server_protocol::TurnStartResponse = to_response(resp)?;
