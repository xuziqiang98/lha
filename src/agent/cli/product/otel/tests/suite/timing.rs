...
use super::super::harness::histogram_data;
...
fn record_duration_records_histogram() -> Result<()> {
    ...
    assert_eq!(sum, 15.0);
    assert_eq!(count, 1);
    let metric = super::super::harness::find_metric(&resource_metrics, "codex.request_latency")
...
fn timer_result_records_success() -> Result<()> {
    ...
    let attrs = attributes_to_map(
        match super::super::harness::find_metric(&resource_metrics, "codex.request_latency")
