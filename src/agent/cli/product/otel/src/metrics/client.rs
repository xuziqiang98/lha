...
use crate::product::otel::metrics::MetricsError;
use crate::product::otel::metrics::Result;
use crate::product::otel::metrics::config::MetricsConfig;
use crate::product::otel::metrics::config::MetricsExporter;
use crate::product::otel::metrics::timer::Timer;
use crate::product::otel::metrics::validation::validate_metric_name;
use crate::product::otel::metrics::validation::validate_tag_key;
use crate::product::otel::metrics::validation::validate_tag_value;
use crate::product::otel::metrics::validation::validate_tags;
...
/// OpenTelemetry metrics client used by LHA.
#[derive(Clone, Debug)]
...
fn build_otlp_metric_exporter(
    ...
    match exporter {
        ...
        OtelExporter::Statsig => build_otlp_metric_exporter(
            crate::product::otel::config::resolve_exporter(&OtelExporter::Statsig),
            temporality,
        ),
        OtelExporter::OtlpGrpc {
            ...
            let header_map = crate::product::otel::otlp::build_header_map(&headers);
            ...
            let tls_config = match tls.as_ref() {
                Some(tls) => crate::product::otel::otlp::build_grpc_tls_config(
                    &endpoint,
                    base_tls_config,
                    tls,
                )
        ...
        OtelExporter::OtlpHttp {
            ...
            if let Some(tls) = tls.as_ref() {
                let client = crate::product::otel::otlp::build_http_client(
                    tls,
