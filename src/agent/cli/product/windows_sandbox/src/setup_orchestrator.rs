...
use crate::product::windows_sandbox::allow::AllowDenyPaths;
use crate::product::windows_sandbox::allow::compute_allow_paths;
use crate::product::windows_sandbox::logging::log_note;
use crate::product::windows_sandbox::policy::SandboxPolicy;
use crate::product::windows_sandbox::setup_error::SetupErrorCode;
use crate::product::windows_sandbox::setup_error::SetupFailure;
use crate::product::windows_sandbox::setup_error::clear_setup_error_report;
use crate::product::windows_sandbox::setup_error::failure;
use crate::product::windows_sandbox::setup_error::read_setup_error_report;
use anyhow::Context;
use anyhow::Result;
use anyhow::anyhow;
use base64::Engine;
...
pub fn sandbox_dir(lha_home: &Path) -> PathBuf {
    lha_home.join(".sandbox")
}

pub fn sandbox_secrets_dir(lha_home: &Path) -> PathBuf {
    lha_home.join(".sandbox-secrets")
}

pub fn setup_marker_path(lha_home: &Path) -> PathBuf {
    sandbox_dir(lha_home).join("setup_marker.json")
}

pub fn sandbox_users_path(lha_home: &Path) -> PathBuf {
    sandbox_secrets_dir(lha_home).join("sandbox_users.json")
}
...
pub fn run_setup_refresh(
    ...
    let cwd = std::env::current_dir().unwrap_or_else(|_| lha_home.to_path_buf());
    log_note(
        &format!(
...
fn find_setup_exe() -> PathBuf {
    std::env::current_exe().unwrap_or_else(|_| PathBuf::from("lha.exe"))
}
...
fn report_helper_failure(
    ...
    match read_setup_error_report(lha_home) {
...
fn run_setup_exe(payload: &ElevationPayload, needs_elevation: bool, lha_home: &Path) -> Result<()> {
    ...
    let cleared_report = match clear_setup_error_report(lha_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
                &format!(
    ...
    if !needs_elevation {
        ...
        if let Err(err) = clear_setup_error_report(lha_home) {
            log_note(
                &format!(
    ...
    let exe_w = crate::product::windows_sandbox::winutil::to_wide(&exe);
    let params = format!(
        "{} {}",
        quote_arg("__windows-sandbox-setup"),
        quote_arg(&payload_b64)
    );
    let params_w = crate::product::windows_sandbox::winutil::to_wide(params);
    let verb_w = crate::product::windows_sandbox::winutil::to_wide("runas");
...
pub fn run_elevated_setup(
    ...
    let sbx_dir = sandbox_dir(lha_home);
    ...
    run_setup_exe(&payload, needs_elevation, lha_home)
}
...
fn build_payload_roots(
    ...
    let write_roots = filter_sensitive_write_roots(write_roots, lha_home);
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, lha_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to LHA_HOME or anything under LHA_HOME/.sandbox.
    // These locations contain sandbox control/state and must remain tamper-resistant.
    let lha_home_key = crate::product::windows_sandbox::audit::normalize_path_key(lha_home);
    let sbx_dir_key =
        crate::product::windows_sandbox::audit::normalize_path_key(&sandbox_dir(lha_home));
    let sbx_dir_prefix = format!("{}/", sbx_dir_key.trim_end_matches('/'));
    let secrets_dir_key =
        crate::product::windows_sandbox::audit::normalize_path_key(&sandbox_secrets_dir(lha_home));
    let secrets_dir_prefix = format!("{}/", secrets_dir_key.trim_end_matches('/'));

    roots.retain(|root| {
        let key = crate::product::windows_sandbox::audit::normalize_path_key(root);
        key != lha_home_key
            && key != sbx_dir_key
