use crate::product::git_utils::GitBaselineDiff;
use crate::product::git_utils::diff_since_latest_init;
use crate::product::git_utils::ensure_git_baseline_repository;
use crate::product::git_utils::reset_git_repository;
use anyhow::Context;
use std::path::Path;
...
pub async fn write_workspace_diff(root: &Path, diff: &GitBaselineDiff) -> anyhow::Result<()> {
    let path = root.join(crate::product::memories_write::workspace_diff::FILENAME);
...
fn append_bounded_diff(rendered: &mut String, diff: &str) {
    if diff.len() <= crate::product::memories_write::workspace_diff::MAX_BYTES {
    ...
    let boundary = previous_char_boundary(
        diff,
        crate::product::memories_write::workspace_diff::MAX_BYTES,
    );
