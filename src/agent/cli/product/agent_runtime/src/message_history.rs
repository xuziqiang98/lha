...
//!
//! The history is stored at `~/.lha/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.lha`.
...
fn history_filepath(config: &Config) -> PathBuf {
    let mut path = config.lha_home.clone();
...
pub(crate) async fn append_entry(
    ...
    // Resolve `~/.lha/history.jsonl` and ensure the parent directory exists.
...
mod tests {
    ...
    async fn append_entry_trims_history_when_beyond_max_bytes() {
        ...
        let history_path = lha_home.path().join("history.jsonl");
