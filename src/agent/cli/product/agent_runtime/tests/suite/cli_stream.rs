...
use crate::product::agent::protocol::GitInfo;
use crate::test_support::cargo_bin::find_resource;
use crate::test_support::core::fs_wait;
...
async fn chat_mode_stream_cli() {
    ...
    let bin = crate::test_support::cargo_bin::cargo_bin("lha").unwrap();
...
async fn exec_cli_applies_model_instructions_file() {
    ...
    let resp_mock =
        crate::test_support::core::responses::mount_sse_once(&server, sse.to_string()).await;
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    let bin2 = crate::test_support::cargo_bin::cargo_bin("lha").unwrap();
...
async fn integration_git_info_unit_test() {
    ...
    let git_info = crate::product::agent::git_info::collect_git_info(&git_repo).await;
