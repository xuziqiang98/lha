...
use crate::product::agent::protocol::ReviewCodeLocation;
use crate::product::agent::protocol::ReviewFinding;
use crate::product::agent::protocol::ReviewLineRange;
...
async fn review_op_emits_lifecycle_and_review_output() {
    ...
    for line in text.lines() {
        ...
        if let RolloutItem::TranscriptItem(item) = rl.item
            && let TranscriptItem::Message { role, content, .. } = item
        {
            if role == "user" {
                for c in &content {
