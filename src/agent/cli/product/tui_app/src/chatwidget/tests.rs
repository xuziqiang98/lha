...
const CYBERGYM_FUZZER_CJK_ASCII_ORDER_TEXT: &str = concat!(
    "对，**这里大多数 fuzzer 是和目标代码编译/链接到一起的**，尤其是 OSS-Fuzz/libFuzzer 风格的 target。\n",
    "\n",
    "它不是这种模型：\n",
    "\n",
    "```text\n",
    "fuzzer  ->  启动/驱动一个外部 project_A 二进制\n",
    "```\n",
    "\n",
    "而更像这种模型：\n",
    "\n",
    ...
    "fuzzer_A 可执行文件 =\n",
    "  fuzzing engine\n",
    "  + harness: LLVMFuzzerTestOneInput(data, size)\n",
    "  + project_A 的源码/库\n",
    "  + sanitizer runtime\n",
    "```\n",
    "\n",
    "所以 `cybergym-server-data/.../out/<fuzzer>` 本身就是“目标程序 + fuzz harness”的组合体。\n",
    "\n",
    "和 AFL 的区别：\n",
    "\n",
    "- **AFL 常见模式**：你可以 `afl-fuzz -i corpus -o out -- ./target @@`，`target` 是外部二进制，AFL 负责变异输入、喂给目标。\n",
    "- **libFuzzer/OSS-Fuzz 常见模式**：fuzzer target 自己就是可执行文件，入口是 `LLVMFuzzerTestOneInput()`，运行方式是 `./fuzzer corpus_dir` 或 `./fuzzer poc_file`。\n",
    "- **CyberGym 这里**：验证时通常直接跑 `/out/<fuzzer> /testcase`，所以它用的是已经编译好的 pinned target 快照。\n",
    "\n",
    "当然，不是说完全不能用 AFL 模式。可以，但前提变了：\n",
    "\n",
    "1. 如果这个任务的 fuzzer 本来就是 AFL target  \n",
    "   例如某些 wrapper 里有 `FUZZING_ENGINE=afl`，那它可能可以按 AFL/AFL++ 风格跑。\n",
    "\n",
    "2. 如果你想 AFL fuzz 最新 `project_A`  \n",
    "   需要重新编译最新 `project_A`，让它带 AFL instrumentation，并准备合适的命令行入口，比如：\n",
    "\n",
    "   ```bash\n",
    "   afl-fuzz -i seeds -o findings -- ./project_A_fuzz_target @@\n",
    "   ```\n",
    "\n",
    "3. 如果你想复用 CyberGym 的 `fuzzer_A` harness  \n",
    "   不是“指定最新 project_A 二进制”，而是把 harness 源码移植到最新版项目，然后重新编译成新的 fuzz target。\n",
    "\n",
    "4. 如果项目原本就是命令行 parser  \n",
    "   可能可以直接 AFL fuzz CLI，例如：\n",
    "\n",
    "   ```bash\n",
    "   afl-fuzz -i seeds -o out -- ./parser @@\n",
    "   ```\n",
    "\n",
    "   但这和 CyberGym 的 `LLVMFuzzerTestOneInput` harness 不一定等价。\n",
    "\n",
    "所以答案是：\n",
    "\n",
    "**是的，这些 fuzzer 通常已经和目标编译到一起了。不能像 AFL 那样随便指定另一个外部二进制来 fuzz。**\n",
    "\n",
    "更精确地说：\n",
    "\n",
    "- 现成 `cybergym-server-data/.../out/fuzzer_A` 测的是它内置的那份 `project_A` 快照。\n",
    "- 要测最新版 `project_A`，需要用 `fuzzer_A` 的源码/harness 重新编译。\n",
    "- 如果想走 AFL 的“指定二进制”模式，需要单独构建 AFL target，而不是直接复用 CyberGym 现成 libFuzzer 二进制。",
);
...
fn assert_cybergym_fuzzer_cjk_ascii_order(rendered: &str, context: &str) {
    for needle in [
        "“目标程序 + fuzz harness”的组合体",
        "3. 如果你想复用 CyberGym 的 fuzzer_A harness",
        "+ harness: LLVMFuzzerTestOneInput(data, size)",
        "FUZZING_ENGINE=afl",
    ] {
        assert!(
            rendered.contains(needle),
            "{context} missing correct CyberGym fuzzer text {needle:?}: {rendered:?}"
        );
    }
    for stale in ["fuzz”的 harness组合体", "3.你 如果", "harness组合体"] {
        assert!(
            !rendered.contains(stale),
            "{context} rendered known CyberGym fuzzer corruption {stale:?}: {rendered:?}"
        );
    }
}
...
#[tokio::test]
async fn structured_reasoning_reconciles_completion_only_summary_before_raw_delta() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;
    chat.config.show_raw_agent_reasoning = true;
    let thread_id = ThreadId::new();
    let item_id = "reasoning-raw-before-summary";
    let summary = "**Checking tests**\n\nSummary body.";
    let raw = "raw detail";
    ...
    let summary_index = rendered
        .find("Summary body.")
        .expect("summary body should be rendered");
    let raw_index = rendered
        .find(raw)
        .expect("raw reasoning should be rendered");

    assert_eq!(cells.len(), 1);
    assert_eq!(rendered.matches("Summary body.").count(), 1);
    assert_eq!(rendered.matches(raw).count(), 1);
    assert!(summary_index < raw_index);
    assert!(!rendered.contains("Checking tests"));
}

#[tokio::test]
async fn structured_reasoning_keeps_standalone_bold_raw_content() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;
    chat.config.show_raw_agent_reasoning = true;
    let thread_id = ThreadId::new();
    let item_id = "reasoning-bold-raw";
    let raw = "**Raw detail**";
    ...
    assert_eq!(history_cells, 1);
    assert!(display.contains("Raw detail"), "display: {display:?}");
    assert!(
        transcript.contains("Raw detail"),
...
#[tokio::test]
async fn legacy_raw_reasoning_keeps_standalone_bold_content() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;
    chat.config.show_raw_agent_reasoning = true;
    let raw = "**Raw detail**";

    chat.handle_codex_event(Event {
        id: "legacy-bold-raw".into(),
        msg: EventMsg::AgentReasoningRawContentDelta(AgentReasoningRawContentDeltaEvent {
            delta: "**Raw detail".into(),
        }),
    });
...
#[tokio::test]
async fn replayed_structured_raw_reasoning_keeps_standalone_bold_content_when_enabled() {
    let thread_id = ThreadId::new();
    let raw = "**replayed raw**";

    let (mut hidden_chat, mut hidden_rx, _op_rx) = make_chatwidget_manual(None).await;
    hidden_chat.handle_codex_event_replay(Event {
        id: "replayed-bold-raw".into(),
        msg: EventMsg::ItemCompleted(ItemCompletedEvent {
            thread_id,
            turn_id: "turn-1".into(),
            item: TurnItem::Reasoning(ReasoningItem {
    ...
    let hidden = drain_insert_history(&mut hidden_rx)
        ...
        .map(|lines| lines_to_single_string(lines))
        .collect::<String>();
    assert!(!hidden.contains("replayed raw"));
    ...
    let visible = drain_insert_history(&mut visible_rx)
        ...
        .map(|lines| lines_to_single_string(lines))
        .collect::<String>();
    assert_eq!(visible.matches("replayed raw").count(), 1);
}
...
#[tokio::test]
async fn final_agent_message_preserves_cybergym_fuzzer_cjk_ascii_order() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;
    let width = 180;
    let height = 48;
    let mut terminal = crate::product::tui_app::custom_terminal::Terminal::with_options(
    ...
    chat.handle_codex_event(Event {
        id: "cybergym-fuzzer-order-turn".into(),
        msg: EventMsg::TurnStarted(TurnStartedEvent {
            model_context_window: None,
            identity_kind: IdentityKind::Nobody,
        }),
    ...
    chat.handle_codex_event(Event {
        id: "cybergym-fuzzer-order-final".into(),
        msg: EventMsg::AgentMessage(AgentMessageEvent {
            message: CYBERGYM_FUZZER_CJK_ASCII_ORDER_TEXT.to_string(),
            memory_citation: None,
        }),
    });
    ...
    for event in drain_events(&mut rx) {
        if let Some(cell) = into_insert_history_cell(event) {
            ...
            if cell.as_any().is::<AgentMessageCell>() {
                saw_agent_message = true;
                assert_cybergym_fuzzer_cjk_ascii_order(
                    &rendered,
                    "CyberGym fuzzer AgentMessage cell",
                );
            }
            let cell: Arc<dyn HistoryCell> = Arc::from(cell);
            chat.insert_transcript_cell(cell);
    ...
    assert!(
        saw_agent_message,
        "expected CyberGym fuzzer AgentMessage cell"
    );

    let screen = render_chat_to_vt100_screen(&chat, &mut terminal);
    assert_cybergym_fuzzer_cjk_ascii_order(&screen, "CyberGym fuzzer VT100 screen");
}
