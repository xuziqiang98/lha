...
pub(crate) fn append_markdown(
    ...
    let rendered = crate::product::tui_app::markdown_render::render_markdown_text_with_width(
        markdown_source,
        width,
    );
    crate::product::tui_app::render::line_utils::push_owned_lines(&rendered.lines, lines);
}
...
mod tests {
    ...
    fn append_markdown_preserves_full_text_line() {
        let src = "Hi! How can I help with this repository today? Want me to explore the repo, run tests, or work on a specific change?\n";
        let mut out = Vec::new();
        append_markdown(src, None, &mut out);
        assert_eq!(
            out.len(),
        ...
        assert_eq!(
            rendered,
            "Hi! How can I help with this repository today? Want me to explore the repo, run tests, or work on a specific change?"
        );
    }
