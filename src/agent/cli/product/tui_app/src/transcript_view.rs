...
use crate::product::tui_app::history_cell::is_proposed_plan_cell;
#[cfg(test)]
use crate::product::tui_app::history_cell::new_plan_update;
#[cfg(test)]
use crate::product::tui_app::history_cell::new_proposed_plan;
#[cfg(test)]
use crate::product::tui_app::history_cell::new_proposed_plan_stream;
#[cfg(test)]
use crate::product::tui_app::history_cell::new_user_prompt;
#[cfg(test)]
use crate::product::tui_app::history_cell::proposed_plan_trailing_body_gap_line;
use crate::product::tui_app::history_cell::render_line_backgrounds;
...
use crate::product::tui_app::style::transcript_selection_style;
use crate::product::tui_app::style::user_message_style;
use crate::product::tui_app::terminal_palette;
use crate::product::tui_app::transcript_selection::TranscriptSelection;
...
impl TranscriptView {
    ...
    fn selection_point_from_position(
        ...
        if line_index >= self.last_total_lines {
            return None;
        }
...
mod tests {
    ...
    #[test]
    fn live_tail_content_change_recomputes() {
    ...
    #[test]
    fn repeated_live_tail_revision_changes_recompute_tail() {
        let mut view = TranscriptView::new(Vec::new(), TranscriptRenderMode::Display);
        let calls = std::cell::Cell::new(0usize);
    ...
    #[test]
    fn live_tail_animation_tick_recomputes_tail() {
        let mut view = TranscriptView::new(Vec::new(), TranscriptRenderMode::Display);
        let first =
    ...
    fn mouse_down_below_top_anchored_content_is_ignored() {
        ...
        assert_eq!(outcome, TranscriptMouseOutcome::Ignored);
        assert!(!view.selection_active_for_test());
    }
    ...
    fn selection_copy_assistant_markdown_keeps_single_blank_lines() {
        let mut view = TranscriptView::new_transcript(vec![Arc::new(
            crate::product::tui_app::history_cell::AgentMessageCell::new(
                vec![
                    "# Title".into(),
                    Line::default(),
                    "## Summary".into(),
    ...
    fn selection_copy_matches_rendered_row_after_leading_blank_agent_line() {
        ...
        let mut view = TranscriptView::new_transcript(vec![Arc::new(
            crate::product::tui_app::history_cell::AgentMessageCell::new(lines, false),
        )]);
    ...
    fn short_transcript_growth_keeps_existing_rows_stable() {
        ...
        let mut grown = Buffer::empty(Rect::new(0, 0, 20, 10));
        view.sync_live_tail(
            area.width,
            Some(TranscriptLiveTailKey::new(
                TranscriptLiveTailSource::ActiveCell,
            ...
            |_| {
                TranscriptView::live_tail_from_lines(vec![
                    "stable first row".into(),
                    "new second row".into(),
                ])
            },
        );
        view.render_inline(area, &mut grown);
        let grown_first_row = (area.y..area.bottom())
            .find(|y| {
                (area.x..area.right())
