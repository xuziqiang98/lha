...
//!
//! For the detailed burst state machine, see `src/agent/cli/product/tui_app/src/bottom_pane/paste_burst.rs`.
...
use crate::product::tui_app::key_hint::has_ctrl_or_alt;
...
use crate::product::agent::terminal::Multiplexer;
...
use crate::product::protocol::custom_prompts::PROMPTS_CMD_PREFIX;
...
use crate::product::tui_app::bottom_pane::paste_burst::FlushResult;
use crate::product::tui_app::bottom_pane::prompt_args::expand_custom_prompt;
use crate::product::tui_app::bottom_pane::prompt_args::expand_if_numeric_with_positional_args;
use crate::product::tui_app::bottom_pane::prompt_args::parse_slash_name;
use crate::product::tui_app::bottom_pane::prompt_args::prompt_argument_names;
use crate::product::tui_app::bottom_pane::prompt_args::prompt_command_with_arg_placeholders;
use crate::product::tui_app::bottom_pane::prompt_args::prompt_has_numeric_placeholders;
use crate::product::tui_app::buddy;
use crate::product::tui_app::buddy::layout::BuddyLayoutMode;
use crate::product::tui_app::buddy::state::BuddyState;
use crate::product::tui_app::render::Insets;
use crate::product::tui_app::render::RectExt;
...
use crate::product::agent::connectors::AppInfo;
...
use crate::product::tui_app::bottom_pane::textarea::TextArea;
use crate::product::tui_app::bottom_pane::textarea::TextAreaState;
use crate::product::tui_app::clipboard_paste::normalize_pasted_path;
use crate::product::tui_app::clipboard_paste::pasted_image_format;
...
impl ChatComposer {
    ...
    pub(crate) fn new_with_config(
        ...
        let mut this = Self {
            ...
            is_zellij: matches!(
                crate::product::agent::terminal::terminal_info().multiplexer,
    ...
    pub(crate) fn buddy(&self) -> Option<&crate::product::tui_app::buddy::model::Buddy> {
    ...
    fn mention_items(&self) -> Vec<MentionItem> {
            ...
            && let Some(snapshot) = self.connectors_snapshot.as_ref()
        {
            for connector in &snapshot.connectors {
                ...
                let slug = crate::product::agent::connectors::connector_mention_slug(connector);
...
impl ChatComposer {
    pub(crate) fn render_with_mask(&self, area: Rect, buf: &mut Buffer, mask_char: Option<char>) {
        ...
        match &self.active_popup {
            ...
            ActivePopup::None => {
                let footer_props = self.footer_props();
                let show_cycle_hint = self.identity_indicator.is_some();
...
mod tests {
    ...
    use crate::product::tui_app::bottom_pane::AppEventSender;
    use crate::product::tui_app::bottom_pane::ChatComposer;
    use crate::product::tui_app::bottom_pane::InputResult;
    use crate::product::tui_app::bottom_pane::chat_composer::AttachedImage;
    use crate::product::tui_app::bottom_pane::chat_composer::LARGE_PASTE_CHAR_THRESHOLD;
    use crate::product::tui_app::bottom_pane::prompt_args::PromptArg;
    use crate::product::tui_app::bottom_pane::prompt_args::extract_positional_args_for_prompt_line;
    use crate::product::tui_app::bottom_pane::textarea::TextArea;
    ...
    fn wide_buddy_layout_centers_pixel_dragon_sprite_on_name_axis() {
        ...
        composer
            .buddy_state
            .set_buddy_for_test(crate::product::tui_app::buddy::model::Buddy {
                name: "Pixel".to_string(),
                species: crate::product::agent::config::types::BuddySpecies::Dragon,
                eye: crate::product::agent::config::types::BuddyEye::Cross,
                hat: crate::product::agent::config::types::BuddyHat::Propeller,
                rarity: crate::product::agent::config::types::BuddyRarity::Epic,
                shiny: false,
                personality: "terminal philosopher".to_string(),
                stats: crate::product::tui_app::buddy::model::BuddyStats {
                    debugging: 100,
                    patience: 45,
                    chaos: 50,
                    wisdom: 49,
                ...
                identity_kind: crate::product::protocol::config_types::IdentityKind::Nobody,
            });
...
程序员 与 Unicode 同行";
    ...
    #[test]
    fn slash_goal_with_args_dispatches_command_and_does_not_submit_literal_text() {
        use crossterm::event::KeyCode;
        use crossterm::event::KeyEvent;
        use crossterm::event::KeyModifiers;
        ...
        match result {
            InputResult::CommandWithArgs(cmd, args) => {
                ...
                assert_eq!(args, "finish migration");
            }
            InputResult::Command(cmd) => {
                panic!("expected command dispatch with args for '/goal', got {cmd:?}")
            }
            InputResult::Submitted { text, .. } => {
                panic!("expected command dispatch, but composer submitted literal text: {text}")
            }
