...
pub struct TuiEventStream<S: EventSource + Default + Unpin = CrosstermEventSource> {
    ...
    #[cfg(unix)]
    suspend_context: crate::product::tui_app::tui::job_control::SuspendContext,
    #[cfg(unix)]
...
impl<S: EventSource + Default + Unpin> TuiEventStream<S> {
    pub fn new(
        ...
        terminal_focused: Arc<AtomicBool>,
        #[cfg(unix)] suspend_context: crate::product::tui_app::tui::job_control::SuspendContext,
        #[cfg(unix)] alt_screen_active: Arc<AtomicBool>,
        #[cfg(unix)] mouse_capture_enabled: bool,
    ) -> Self {
    ...
    fn map_crossterm_event(&mut self, event: Event) -> Option<TuiEvent> {
        match event {
            Event::Key(key_event) => {
                #[cfg(unix)]
                if crate::product::tui_app::tui::job_control::SUSPEND_KEY.is_press(key_event) {
                    let _ = self
                        .suspend_context
                        .suspend(&self.alt_screen_active, self.mouse_capture_enabled);
            ...
            Event::FocusGained => {
                ...
                crate::product::tui_app::terminal_palette::requery_default_colors();
                Some(TuiEvent::Draw)
            }
            Event::FocusLost => {
