...
use crate::product::tui_app::history_cell::padded_emoji;
...
use crate::product::tui_app::updates;
...
pub(crate) async fn run_update_prompt_if_needed(
    ...
    let Some(update_action) = crate::product::tui_app::update_action::get_update_action() else {
        return Ok(UpdatePromptOutcome::Continue);
    };
...
impl WidgetRef for &UpdatePromptScreen {
    fn render_ref(&self, area: Rect, buf: &mut Buffer) {
        ...
        column.push(
            Line::from(vec![
                "Release notes: ".dim(),
                LHA_RELEASE_NOTES_URL.dim().underlined(),
            ])
