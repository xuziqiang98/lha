...
use crate::product::tui_app::app::App;
...
impl App {
    ...
    fn overlay_forward_event(&mut self, tui: &mut tui::Tui, event: TuiEvent) -> Result<()> {
            ...
            && let Some(Overlay::Transcript(t)) = &mut self.overlay
        {
            ...
            let overlay_width = tui.terminal.size()?.width.max(1);
            t.sync_live_tail(overlay_width, live_tail_key, |w| {
            ...
            if schedule_drag_frame && !close_overlay {
                tui.frame_requester().schedule_frame_in(
                    crate::product::tui_app::chatwidget::DRAG_AUTOSCROLL_INTERVAL,
                );
            }
            if !close_overlay
...
fn trim_transcript_cells_to_nth_user(
    transcript_cells: &mut Vec<Arc<dyn crate::product::tui_app::history_cell::HistoryCell>>,
...
pub(crate) fn user_count(
    cells: &[Arc<dyn crate::product::tui_app::history_cell::HistoryCell>],
) -> usize {
...
fn user_positions_iter(
    ...
    let type_of = |cell: &Arc<dyn crate::product::tui_app::history_cell::HistoryCell>| {
        cell.as_any().type_id()
    };
