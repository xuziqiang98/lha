...
pub fn arg0_dispatch() -> Option<TempDir> {
    ...
    if argv1 == CODEX_APPLY_PATCH_ARG1 {
        ...
        let exit_code = match patch_arg {
            Some(patch_arg) => {
                ...
                match crate::product::apply_patch::apply_patch(&patch_arg, &mut stdout, &mut stderr)
                {
                    Ok(()) => 0,
                    Err(_) => 1,
                }
    ...
    match prepend_path_entry_for_codex_aliases() {
        Ok(path_entry) => {
            dispatch_single_binary_alias(exe_name, Some(&path_entry));
            Some(path_entry)
        }
        Err(err) => {
            ...
            dispatch_single_binary_alias(exe_name, None);
            None
        }
    }
}

fn normalize_exe_name(name: &str) -> &str {
...
fn dispatch_single_binary_alias(exe_name: &str, _path_entry: Option<&TempDir>) {
    match exe_name {
        ...
        APP_SERVER_ARG0 => {
            exit_after_block_on(async move {
                let loader_overrides = crate::product::agent::config_loader::LoaderOverrides {
                    managed_config_path: managed_config_path_from_debug_env(),
                    ..Default::default()
                };
                crate::product::app_server::run_main(
                    linux_sandbox_exe(),
                    crate::product::common::CliConfigOverrides::default(),
                    loader_overrides,
                    false,
                )
                .await
        ...
        MCP_SERVER_ARG0 => {
            exit_after_block_on(async move {
                crate::product::mcp_server::run_main(
                    linux_sandbox_exe(),
                    crate::product::common::CliConfigOverrides::default(),
                )
                .await
        ...
        STDIO_TO_UDS_ARG0 => {
            let Some(socket_path) = std::env::args_os().nth(1) else {
                eprintln!("Error: {STDIO_TO_UDS_ARG0} requires a SOCKET_PATH argument.");
                std::process::exit(1);
            };
...
fn managed_config_path_from_debug_env() -> Option<PathBuf> {
    ...
    {
        if let Ok(value) = std::env::var("CODEX_APP_SERVER_MANAGED_CONFIG_PATH") {
...
fn exit_after_block_on<F>(future: F) -> !
where
...
/// [`crate::product::linux_sandbox::run_main`] (which never returns). Otherwise we:
///
...
///     constructing [`crate::product::agent::config::Config`].
///
...
fn load_dotenv() {
    if let Ok(lha_home) = crate::product::agent::config::find_lha_home()
...
pub fn prepend_path_entry_for_codex_aliases() -> std::io::Result<TempDir> {
    let lha_home = crate::product::agent::config::find_lha_home()?;
