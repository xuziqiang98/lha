...
use crate::product::utils_pty::process::ChildTerminator;
use crate::product::utils_pty::process::ProcessHandle;
use crate::product::utils_pty::process::SpawnedProcess;
...
impl ChildTerminator for PipeChildTerminator {
    fn kill(&mut self) -> io::Result<()> {
        #[cfg(unix)]
        {
            crate::product::utils_pty::process_group::kill_process_group(self.process_group_id)
        }
...
async fn spawn_process_with_stdin_mode(
    ...
    unsafe {
        command.pre_exec(move || {
            ...
            #[cfg(target_os = "linux")]
            crate::product::utils_pty::process_group::set_parent_death_signal(parent_pid)?;
            Ok(())
        });
    }
    #[cfg(not(unix))]
