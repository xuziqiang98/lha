...
pub fn load_sse_fixture(path: impl AsRef<std::path::Path>) -> String {
    let path = agent_runtime_test_resource(path.as_ref());
...
pub fn load_sse_fixture_with_id(path: impl AsRef<std::path::Path>, id: &str) -> String {
    let full_path = agent_runtime_test_resource(path.as_ref());
...
fn agent_runtime_test_resource(path: &std::path::Path) -> std::path::PathBuf {
    if path.is_absolute() {
    ...
    if let Some(rest) = path_str.strip_prefix("../fixtures/") {
        return manifest_dir
            .join("product/agent_runtime/tests/fixtures")
            .join(rest);
    }
    if let Some(rest) = path_str.strip_prefix("tests/fixtures/") {
        return manifest_dir
            .join("product/agent_runtime/tests/fixtures")
            .join(rest);
    }
...
pub async fn wait_for_event<F>(
    codex: &CodexThread,
    predicate: F,
) -> crate::product::agent::protocol::EventMsg
where
    F: FnMut(&crate::product::agent::protocol::EventMsg) -> bool,
{
...
where
    F: Fn(&crate::product::agent::protocol::EventMsg) -> Option<T>,
{
...
pub fn sandbox_env_var() -> &'static str {
    crate::product::agent::spawn::LHA_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    crate::product::agent::spawn::LHA_SANDBOX_NETWORK_DISABLED_ENV_VAR
}

pub fn format_with_current_shell(command: &str) -> Vec<String> {
    crate::product::agent::shell::default_user_shell().derive_exec_args(command, true)
}
...
pub fn format_with_current_shell_non_login(command: &str) -> Vec<String> {
    crate::product::agent::shell::default_user_shell().derive_exec_args(command, false)
}
...
macro_rules! skip_if_sandbox {
    () => {{
        if ::std::env::var($crate::test_support::core::sandbox_env_var())
            == ::core::result::Result::Ok("seatbelt".to_string())
        {
            eprintln!(
                "{} is set to 'seatbelt', skipping test.",
                $crate::test_support::core::sandbox_env_var()
            );
            return;
        }
    }};
...
pub(crate) use skip_if_sandbox;

macro_rules! skip_if_no_network {
    () => {{
        if ::std::env::var($crate::test_support::core::sandbox_network_env_var()).is_ok() {
            println!(
...
pub(crate) use skip_if_no_network;
